...
| sidecars[] | Optional containers to include as sidecars for the vertica server.  Each entry is a fully formed container spec, similar to the container you would add to a Pod spec.  The operator will automatically add to each container the same volume mounts that are in the vertica server container.<br><br>Here is an example of adding a sidecar name vlogger.<br><pre>sidecars:<br>  - name: vlogger<br>    image: image:tag</pre>| empty |
