...
Each pod uses a PV to store local data. The PV is mounted in the container at `/home/dbadmin/local-data`. You must set permissions on the PV mount to 0775, or the operator might get a "Permissions Denied" error. If the PV was dynamically provisioned, you might need to manually change permissions with `chmod` after it is created.
