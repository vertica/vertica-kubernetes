...
perl -i -0777 -pe "s/(CurOperatorVersion\s*=\s*\")[0-9.\-]+(\")/\${1}$VERSION\${2}/" $REPO_DIR/pkg/meta/labels.go
cd $REPO_DIR
make fmt

logInfo "Changing version in the helm chart"
perl -i -0777 -pe "s/(name:\s*.*verticadb-operator:)[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9]+)?/\${1}$VERSION/g" $REPO_DIR/helm-charts/verticadb-operator/values.yaml
perl -i -0777 -pe "s/^(version:\s+)[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9]+)?/\${1}$VERSION/gm" $REPO_DIR/helm-charts/verticadb-operator/Chart.yaml
perl -i -0777 -pe "s/(verticadb-operator:)[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9]+)?/\${1}$VERSION/g" $REPO_DIR/helm-charts/verticadb-operator/README.md
