...
func MakePodFacts(vrec *VerticaRestorePointsQueryReconciler, prunner cmds.PodRunner) PodFacts {
...
// findExpectedNodeIps will return a list of pods that should have been in the database
// before running db_add_node (which are also called expected nodes)
func (p *PodFacts) findExpectedNodeIps() []string {
	var expectedNodeIps []string

	for _, v := range p.Detail {
		if v.dbExists {
			expectedNodeIps = append(expectedNodeIps, v.podIP)
		}
	}

	return expectedNodeIps
}
