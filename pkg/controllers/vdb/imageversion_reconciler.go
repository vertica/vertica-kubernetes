...
func (v *ImageVersionReconciler) checkNMACertCompatability(vinf *version.Info) ctrl.Result {
	...
	if !vmeta.UseVClusterOps(v.Vdb.Annotations) || v.Vdb.Spec.HTTPSNMATLS.Secret == "" {
		return ctrl.Result{}
	}

	if secrets.IsK8sSecret(v.Vdb.Spec.HTTPSNMATLS.Secret) {
		return ctrl.Result{}
	} else if secrets.IsGSMSecret(v.Vdb.Spec.HTTPSNMATLS.Secret) {
	...
	} else if secrets.IsAWSSecretsManagerSecret(v.Vdb.Spec.HTTPSNMATLS.Secret) {
