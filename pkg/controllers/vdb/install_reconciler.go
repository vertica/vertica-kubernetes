...
func (d *InstallReconciler) Reconcile(ctx context.Context, _ *ctrl.Request) (ctrl.Result, error) {
	...
	return d.doInstall(ctx)
}

// doInstall will go through the install phase.  It will look at the collected
...
func (d *InstallReconciler) doInstall(ctx context.Context) (ctrl.Result, error) {
	...
	// Check if the httpstls.json file needs to be generated by the installer.
	// This file may be needed by the operator, but depends on a bunch of factors:
	// what version the database was created with, deployment type, etc.
	if enabled, err := d.Vdb.IsHTTPSTLSConfGenerationEnabled(); enabled || err != nil {
		if err != nil {
			return ctrl.Result{}, err
		}
...
func (d *InstallReconciler) generateHTTPCerts(ctx context.Context) error {
	for _, p := range d.PFacts.Detail {
		...
		if !p.GetFileExists()[paths.HTTPTLSConfFile] {
			...
			secretName := names.GenNamespacedName(d.Vdb, d.Vdb.Spec.HTTPSNMATLS.Secret)
