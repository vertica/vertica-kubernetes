...
func (h *NMACertRotationReconciler) Reconcile(ctx context.Context, _ *ctrl.Request) (ctrl.Result, error) {
	...
	// Clear TLSCertRotationInProgress and TLSCertRotationInProgress conditions
	conds := []*metav1.Condition{vapi.MakeCondition(vapi.TLSCertRotationInProgress, metav1.ConditionFalse, "Completed"),
		vapi.MakeCondition(vapi.HTTPSCertRotationFinished, metav1.ConditionFalse, "Completed")}
...
func (h *NMACertRotationReconciler) rotateNmaTLSCert(ctx context.Context, newSecret,
	...
	if rotated == 0 {
		...
		if err != nil {
			h.Log.Error(err, "failed to rotate nma cert to "+newSecretName)
