...
var _ = Describe("obj_reconcile", func() {
	...
	Context("When reconciling a VerticaDB CRD", func() {
		It("Should create service objects", func() {
			...
			Expect(foundSvc.Spec.Ports[0].Port).Should(Equal(vdb.Spec.ServiceClientPort))
			Expect(foundSvc.Spec.Ports[1].Port).Should(Equal(vdb.Spec.ServiceHTTPSPort))
		...
		It("should be a no-op if EnableTLSAuth is disabled", func() {
			vdb := vapi.MakeVDB()
			vdb.Annotations[vmeta.EnableTLSAuth] = falseStr
		...
		It("should create the ConfigMap if it does not exist", func() {
			vdb := vapi.MakeVDB()
			vdb.Annotations[vmeta.EnableTLSAuth] = trueStr
