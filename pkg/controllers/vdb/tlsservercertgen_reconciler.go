	...
	for _, s := range secretStruct {
		...
		if secretFieldName == nmaTLSSecret && !h.Vdb.IsHTTPSNMATLSAuthEnabled() {
			if h.Vdb.Spec.HTTPSNMATLS != nil && h.Vdb.Spec.HTTPSNMATLS.Secret != "" {
				// In older operator versions like v25.3.0, nmaTLSSecret was deprecated.
				// We would always set spec.httpsNMATLS.secret even when tls is disabled.
				// If you upgrade to 25.4.0 or later with tls disabled, nmaTLSSecret is back and will autogenerated.
				// The issue is nma is using the secret defined in spec.httpsNMATLS.secret.
				// In this case, instead of generating a new secret, we will use the one defined in spec.httpsNMATLS.secret.
