...
func (h *TLSServerCertGenReconciler) handleLegacyNMATLSSecret(ctx context.Context, secretFieldName string) (bool, error) {
	if secretFieldName != nmaTLSSecret || vmeta.UseTLSAuth(h.Vdb.Annotations) || !h.Vdb.IsDBInitialized() {
		return false, nil
	}
	...
	if h.Vdb.Spec.HTTPSNMATLS != nil &&
		...
		// If you upgrade to 25.3.1 or later with tls disabled, spec.nmaTLSSecret is back and is autogenerated
		// if empty.
		// The issue is when tls is disabled, nma in v25.3.0 got its secret from spec.httpsNMATLS.secret while
		// in v25.3.1+, it assumes the secret is in spec.nmaTLSSecret. To honore that,
