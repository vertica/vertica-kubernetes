...
var _ = Describe("tls_config", func() {
	...
	It("should return HTTPS events", func() {
		...
		Expect(start).To(Equal(events.HTTPSCertRotationStarted))
		Expect(fail).To(Equal(events.HTTPSCertRotationFailed))
		Expect(success).To(Equal(events.HTTPSCertRotationSucceeded))
	})
