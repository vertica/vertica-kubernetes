...
var _ = Describe("nodedetailsvsql", func() {
	It("should parse read-only and sandbox states from node query", func() {
		...
		Expect(nodeDetails.parseNodeState("v_db_node0001|UP|123456|t|sb1|t\n")).Should(Succeed())
		...
		nodeDetails = &NodeDetails{}
		Expect(nodeDetails.parseNodeState("v_db_node0001|UP|7890123|f|sb2|f\n")).Should(Succeed())
		...
		Expect(nodeDetails.parseNodeState("v_db_node0001|UP|123|garbage|t")).Should(Succeed())
	})
