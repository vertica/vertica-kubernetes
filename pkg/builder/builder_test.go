...
var _ = Describe("builder", func() {
	...
	It("configmap should have nma cert secret name and namespace", func() {
		...
		Ω(configMap.Data[NMASecretNameEnv]).Should(Equal(vdb.Spec.HTTPSNMATLS.Secret))
...
func NMACertsVolumeExists(vdb *vapi.VerticaDB, vols []v1.Volume) bool {
	for i := range vols {
		if vols[i].Name == vapi.NMACertsMountName && vols[i].Secret.SecretName == vdb.Spec.HTTPSNMATLS.Secret {
			return true
		}
	}
	return false
