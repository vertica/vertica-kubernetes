...
// mock version of VStopSubcluster() that is invoked inside VClusterOps.StopSubcluster()
...
var _ = Describe("stop_sc_vc", func() {
	ctx := context.Background()

	It("should call vcluster-ops library with stop_sc task", func() {
		dispatcher := mockVClusterOpsDispatcher()
		dispatcher.VDB.Spec.DBName = TestDBName
		err := dispatcher.StopSubcluster(ctx,
			stopsc.WithInitiator(TestInitiatorIP),
			stopsc.WithSubcluster(TestSCName),
		)
