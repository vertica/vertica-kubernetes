...
type HealthWatchdogHostValue struct {
	Host  string
	Value HealthWatchdogValue
}
...
// VHealthWatchdogGet gets the value for a health watchdog module
func (vcc VClusterCommands) VHealthWatchdogGet(options *VHealthWatchdogGetOptions) (*[]HealthWatchdogHostValue,
	error) {
	/*
...
func (vcc VClusterCommands) produceHealthWatchdogGetInstructions(options *VHealthWatchdogGetOptions,
	healthWatchdogValues *[]HealthWatchdogHostValue) ([]clusterOp, error) {
	...
	nmaHealthWatchdogGetData.ModuleName = options.ModuleName
