...
type VListPackagesOptions struct {
	...
	// - If database is DOWN: Lists packages from filesystem only (no status)
}
...
func (vcc VClusterCommands) VListPackages(options *VListPackagesOptions) (*ListPackageStatus, error) {
	...
	_ = vcc.getVDBFromRunningDBIncludeSandbox(&vdb, &options.DatabaseOptions, util.MainClusterSandbox)
