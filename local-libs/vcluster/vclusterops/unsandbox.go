...
func (vcc *VClusterCommands) produceUnsandboxSCInstructions(options *VUnsandboxOptions, info *ProcessedVDBInfo,
	...
	if options.DemotionRequired {
		instructions, err = buildDemoteScInstructions(instructions, vdb, info.upSandboxCousins, info.SandboxName, options)
		if err != nil {
			return instructions, err
		}
	}
...
func buildDemoteScInstructions(instructions []clusterOp,
	...
	httpsDemoteScOp, err := makeHTTPSDemoteSubclusterOp([]string{sandboxCousins[0]}, options.usePassword,
	...
	rebalanceOp, err := makeHTTPSRebalanceSubclusterShardsOp(
		[]string{sandboxCousins[0]}, options.usePassword, options.UserName, options.Password, options.SCName)
	if err != nil {
		return instructions, err
	}
	// Poll for active subscriptions for other sandbox up primary nodes
	httpsPollSubscriptionStateOp, e := makeHTTPSPollSubscriptionStateOp([]string{sandboxCousins[0]},
