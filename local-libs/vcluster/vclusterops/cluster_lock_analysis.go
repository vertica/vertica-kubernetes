...
func (opt *VClusterHealthOptions) fillEventSessionAndTxnInfo(logger vlog.Printer, upHosts []string) {
	...
	for i := range events {
		...
		for j := range *event.LockWaitEvents {
			...
			sessionMap[waitEvent.SessionID] = dcSessionStart{}
			txnMap[waitEvent.TxnID] = dcTransactionStart{}
		}
		for j := range *event.LockHoldEvents {
			holdEvent := &(*event.LockHoldEvents)[j]
			sessionMap[holdEvent.SessionID] = dcSessionStart{}
			txnMap[holdEvent.TxnID] = dcTransactionStart{}
		}
	}
