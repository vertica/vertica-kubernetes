...
func (options *VClusterHealthOptions) buildLockCascadeGraph(logger vlog.Printer,
	...
	sort.Slice(options.LockEventCascade, func(i, j int) bool {
		return options.LockEventCascade[i].LockWaitEvents[len(options.LockEventCascade[i].LockWaitEvents)-1].StartTime <
			options.LockEventCascade[j].LockWaitEvents[len(options.LockEventCascade[j].LockWaitEvents)-1].StartTime
	})

	for i, item := range options.LockEventCascade {
		earliestEvent := item.LockWaitEvents[len(item.LockWaitEvents)-1]
		e := options.getLockReleases(logger, upHosts, earliestEvent.StartTime, earliestEvent.Time, i)
		if e != nil {
			return e
		}
	}
...
func (options *VClusterHealthOptions) getLockReleases(logger vlog.Printer, upHosts []string,
	startTime, endTime string, cascadeIndex int) error {
	var instructions []clusterOp
