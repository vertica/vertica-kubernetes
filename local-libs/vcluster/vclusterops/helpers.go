...
func getInitiatorForDBGroupHelper(vdb *VCoordinationDatabase, targetSandbox string, useSecondaryUpNodes bool) string {
	nodeList := vdb.PrimaryUpNodes
	if useSecondaryUpNodes {
		nodeList = vdb.SecondaryUpNodes
	}
...
func getPrimaryUpInitiatorForDBGroup(vdb *VCoordinationDatabase, targetSandbox string) string {
	return getInitiatorForDBGroupHelper(vdb, targetSandbox, false /*use secondary nodes?*/)
}

func getSecondaryUpInitiatorForDBGroup(vdb *VCoordinationDatabase, targetSandbox string) string {
	return getInitiatorForDBGroupHelper(vdb, targetSandbox, true /*use secondary nodes?*/)
}
...
func getInitiatorForDBGroupFromVDB(vdb *VCoordinationDatabase, targetSandbox string) (string, error) {
	if host := getPrimaryUpInitiatorForDBGroup(vdb, targetSandbox); host != "" {
		return host, nil
	}
	if host := getSecondaryUpInitiatorForDBGroup(vdb, targetSandbox); host != "" {
		return host, nil
	}
	return "", fmt.Errorf("cannot find any UP host for the operation")
}
