...
func makeNMACheckActiveSessionsOp(hosts []string, dbName string,
	usePassword bool, userName string, password *string,
	activeSessions *[]nmaActiveSessionDetails, assertNoActiveSessions bool) (nmaCheckActiveSessionsOp, error) {
	...
	if usePassword {
		err := util.ValidateUsernameAndPassword(op.name, usePassword, userName)
		if err != nil {
			return op, err
		}
	...
	err := op.setupRequestBody(userName, dbName, usePassword, password)
	return op, err
}
...
type nmaActiveSessionDetails struct {
...
func (op *nmaCheckActiveSessionsOp) processResult(_ *opEngineExecContext) error {
	...
	for host, result := range op.clusterHTTPRequest.ResultCollection {
		...
		responseObj := []nmaActiveSessionDetails{}
