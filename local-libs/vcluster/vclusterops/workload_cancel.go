...
func (vcc VClusterCommands) produceWorkloadCancelInstructions(options *VWorkloadCancelOptions) ([]clusterOp, error) {
	...
	if len(hosts) == 0 {
		return instructions, fmt.Errorf("found no UP nodes for workload cancel")
	}
	...
	nmaWorkloadCancelOp := nmaWorkloadCancelRequestData{}
	nmaWorkloadCancelOp.DBName = options.DBName
	nmaWorkloadCancelOp.UserName = options.UserName
	nmaWorkloadCancelOp.Password = options.Password
	nmaWorkloadCancelOp.JobID = options.JobID

	nmaWorkloadReplayOp, err := makeNMAWorkloadCancelOp(initiatorHost, options.usePassword, &nmaWorkloadCancelOp)
	if err != nil {
		return instructions, err
	}
