...
func (op *httpsCheckRunningDBOp) processResult(_ *opEngineExecContext) error {
	...
	for host, result := range op.clusterHTTPRequest.ResultCollection {
		...
		if result.isFailing() {
			if result.isHTTPRunning() {
				op.logger.PrintInfo("[%s] Vertica process might still be running on the host %v. "+
					"Hint: you might have to use the NMA endpoint /v1/vertica-process/signal?signal_type=kill "+
					"to kill a hanging Vertica process on this failed host before proceeding with "+
					"future vcluster operations.", op.name, host)
			} else {
				downHosts.Add(host)
				continue
			}
	...
	return op.handleDBRunning(allErrs, outputMsg, upHosts, downHosts, exceptionHosts, mainClusterHosts, sandboxedHosts)
}
