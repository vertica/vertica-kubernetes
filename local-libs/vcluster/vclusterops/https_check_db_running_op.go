...
func (op *httpsCheckRunningDBOp) processResult(_ *opEngineExecContext) error {
	...
	for host, result := range op.clusterHTTPRequest.ResultCollection {
		...
		if httpsRunning.Cardinality() > 0 {
			op.logger.PrintInfo("[%s] https process might still be running on these hosts: %v "+
				"hint: consider terminating the hanging HTTP process on the above hosts"+
				"before proceeding with future vcluster operations", op.name, httpsRunning.ToSlice())
		}
		upHosts[host] = true
