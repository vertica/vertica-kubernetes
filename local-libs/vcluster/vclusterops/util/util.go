...
// GetVerticaPassthroughEnv retrieves environment variables specific to Vertica passthrough
// and returns them as a map of key-value pairs.
func GetVerticaPassthroughEnv() map[string]string {
	env := make(map[string]string)
	passthrough := os.Getenv("VERTICA_VCLUSTER_PASSTHROUGH")
	const maxSplit = 2
	if passthrough != "" {
		...
		for _, pair := range pairs {
			...
			if len(kv) == 0 || kv[0] == "" {
				break // Invalid format: key is missing
			}
