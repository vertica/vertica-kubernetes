...
func (p *vWorkloadPreprocessCall) PreprocessQuery(logger logr.Logger) error {
	...
	reCopyFromFile := regexp.MustCompile(`(?i)^\s*COPY\s+.*\s+FROM\s+'[^']+'`)
	reCopySource := regexp.MustCompile(`(?i)^\s*COPY\s+.*\s+SOURCE\s+\w+\s*\(`)

	switch {
	case reCopyFromFile.MatchString(query):
		// Traditional COPY FROM 'file.csv'
		logger.Info("Detected COPY FROM file", "query", query)
		p.GenerateCopyQueryFromFile(query, logger)

	case reCopySource.MatchString(query):
		// COPY SOURCE KafkaSource(...)
		logger.Info("Detected COPY SOURCE (Kafka)", "query", query)
		p.GenerateCopyQueryFromSource(query, logger)
...
func (p *vWorkloadPreprocessCall) GenerateCopyQueryFromSource(query string, logger logr.Logger) {
