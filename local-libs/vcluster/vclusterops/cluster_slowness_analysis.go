...
func (opt *VClusterHealthOptions) buildCascadeGraph(logger vlog.Printer, upHosts []string) error {
	...
	allSlowEvents, err := opt.loadAllSlowEvents(logger, upHosts, originalStart)
	if err != nil {
		return err
	}
	...
	rootNode, err := buildCascadeTree(slowestEvent, eventMap, 0, maxDepth, sessionIDs, txnIDs)
	if err != nil {
	...
	// print out the sessionIDs and txnIDs keys for debugging
	for id := range sessionIDs {
		fmt.Printf("Found Session ID: %s\n", id)
	}
	for id := range txnIDs {
		fmt.Printf("Found Transaction ID: %s\n", id)
	}
...
func (opt *VClusterHealthOptions) loadAllSlowEvents(logger vlog.Printer,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get slow events: %w", err)
	}
...
func (opt *VClusterHealthOptions) attachSessionTxnInfo(sessionIDs, txnIDs map[string]any, logger vlog.Printer, upHosts []string) error {
	...
	// print out the session and transaction info for debugging
	for id, sess := range sessions {
		fmt.Printf("Found Session ID: %s, Info: %+v\n", id, sess)
	}
	for id, txn := range txns {
		fmt.Printf("Found Transaction ID: %s, Info: %+v\n", id, txn)
	}
...
func (opt *VClusterHealthOptions) attachSessionTxnToNode(i int, node SlowEventNode,
	sessions map[string]*dcSessionStart, txns map[string]*dcTransactionStart) {
	if node.Event != nil {
		...
		if node.PriorHoldEvents != nil {
			for j := range *node.PriorHoldEvents {
				...
				fmt.Println("Attaching session and transaction info to hold event:", holdEvent.getSessionID(), holdEvent.getTxnID())
				fmt.Printf("transaction info:%v\n", txns[holdEvent.getTxnID()])
				opt.attachSessionTxnToEvent(i, holdEvent, sessions, txns)
				fmt.Printf("Hold event after attaching session and txn info:%v txn %v", holdEvent.SessionInfo, holdEvent.TxnInfo)
			}
		}
	}
}
...
func (opt *VClusterHealthOptions) getSessionTxnInfo(sessionIDs, txnIDs map[string]any, logger vlog.Printer,
	upHosts []string) (sessionMap map[string]*dcSessionStart, txnMap map[string]*dcTransactionStart, err error) {
	// If we don't need session or transaction info, return early
	sessionMap = make(map[string]*dcSessionStart)
	txnMap = make(map[string]*dcTransactionStart)
	if !opt.NeedSessionTnxInfo {
		logger.PrintInfo("Skipping session and transaction info retrieval")
		return sessionMap, txnMap, nil
	}
	sessionStr := joinKeys(sessionIDs)
	txnStr := joinKeys(txnIDs)
