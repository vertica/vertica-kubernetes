...
func (vcc VClusterCommands) VRemoveSubcluster(removeScOpt *VRemoveScOptions) (VCoordinationDatabase, error) {
	...
	if needRemoveNodes {
		...
		// The initiator is selected strictly from a sandbox if we are removing the sc from a sandbox
		// else it is a set of primary up nodes from the main cluster.
		initiators, err := removeScOpt.getAllInitiatorsOfTheOperatingDBGroup(&vdb)
		if err != nil {
			return vdb, err
		}
...
func (options *VRemoveScOptions) getAllInitiatorsOfTheOperatingDBGroup(vdb *VCoordinationDatabase) ([]string, error) {
	var initiator string
	var err error
	if options.Sandbox != util.MainClusterSandbox {
		initiator = vdb.getSandboxInitiator(options.Sandbox)
	} else {
...
func (vcc VClusterCommands) dropSubcluster(vdb *VCoordinationDatabase, options *VRemoveScOptions) error {
	...
	initiators, err := options.getAllInitiatorsOfTheOperatingDBGroup(vdb)
	if err != nil {
		return err
	}
