...
const ScrutinizeLogLimitGB = 50 // 50GB in bytes is the limit for individual log size
const ScrutinizeFileLimitGB = 1 // 1GB in bytes is the limit for individual misc file size
...
type VScrutinizeOptions struct {
	...
	LogFileSizeLimit            int // max log file size in GB
	ScrutinizeFileSizeLimit     int // max non-log file size in GB
	...
	logFileSizeLimitBytes        int64             // calculated from exported log file size limit options
	scrutinizeFileSizeLimitBytes int64             // calculated from exported scrutinize file size limit options
}
...
func (options *VScrutinizeOptions) setDefaultValues() {
	...
	options.LogFileSizeLimit = ScrutinizeLogLimitGB
	options.ScrutinizeFileSizeLimit = ScrutinizeFileLimitGB
}
...
func (options *VScrutinizeOptions) setFileLimits(logger vlog.Printer) (err error) {
	if (options.LogFileSizeLimit < 0) || (options.ScrutinizeFileSizeLimit < 0) {
		return fmt.Errorf("file size limit must be 0 (unlimited file size) or greater than 0")
	}
	options.logFileSizeLimitBytes = int64(options.LogFileSizeLimit * oneGB)
	options.scrutinizeFileSizeLimitBytes = int64(options.ScrutinizeFileSizeLimit * oneGB)
	logger.Info("File size limits set", "LogFile", options.logFileSizeLimitBytes, "NonLogFile", options.scrutinizeFileSizeLimitBytes)
	return nil
}
...
func (options *VScrutinizeOptions) analyzeOptions(logger vlog.Printer) (err error) {
	...
	err = options.setFileLimits(logger)
	if err != nil {
		return err
	}
...
func (vcc VClusterCommands) produceScrutinizeInstructions(options *VScrutinizeOptions,
	...
	stageVerticaLogsOp, err := makeNMAStageVerticaLogsOp(options.ID, options.Hosts,
		hostNodeNameMap, hostCatPathMap, options.logFileSizeLimitBytes, options.logAgeMaxHours, options.logAgeMinHours)
	if err != nil {
	...
	stageVerticaNormalFilesOp, err := makeNMAStageFilesOp(options.ID, scrutinizeBatchNormal,
		options.Hosts, hostNodeNameMap, hostCatPathMap, options.scrutinizeFileSizeLimitBytes)
	if err != nil {
		return nil, err
	}
