...
const ScrutinizeMiscFileLimitGB = 1 // 1GB in bytes is the limit for individual misc file size
...
type VScrutinizeOptions struct {
	...
	MiscFileSizeLimit           int // max non-log file size in GB
	...
	miscFileSizeLimitBytes int64             // calculated from exported scrutinize file size limit options
}
...
func (options *VScrutinizeOptions) setDefaultValues() {
	...
	options.MiscFileSizeLimit = ScrutinizeMiscFileLimitGB
}
...
func (options *VScrutinizeOptions) setFileLimits(logger vlog.Printer) (err error) {
	if (options.LogFileSizeLimit < 0) || (options.MiscFileSizeLimit < 0) {
	...
	options.miscFileSizeLimitBytes = int64(options.MiscFileSizeLimit * oneGB)
	logger.Info("File size limits set", "LogFile", options.logFileSizeLimitBytes, "NonLogFile", options.miscFileSizeLimitBytes)
	return nil
}
...
func (vcc VClusterCommands) produceScrutinizeInstructions(options *VScrutinizeOptions,
	...
	stageVerticaNormalFilesOp, err := makeNMAStageFilesOp(options.ID, scrutinizeBatchNormal,
		options.Hosts, hostNodeNameMap, hostCatPathMap, options.miscFileSizeLimitBytes)
	if err != nil {
		return nil, err
	}
