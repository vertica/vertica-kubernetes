...
// Package name validation: follows Vertica unquoted identifier rules
...
func makeCmdListPackages() *cobra.Command {
	...
	cmd := makeBasicCobraCmd(
		...
		`Lists all available packages in /opt/vertica/packages.

If database credentials are provided, the command will also show 
whether each package is installed in the specified database.

Examples:
  # List all available packages (no status information)
...
func (c *CmdListPackages) validateParse() error {
	...
	// Only validate certs/password if database name is provided
	if c.listPkgOpts.DBName != "" {
		if !c.usePassword() {
		...
		// Only validate base options if database name is provided
...
func (c *CmdListPackages) Run(vcc vclusterops.ClusterCommands) error {
	...
	vcc.DisplayInfo("Successfully listed packages")
