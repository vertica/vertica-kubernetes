...
`,
		[]string{dbNameFlag, configFlag, hostsFlag, ipv6Flag, passwordFlag, sandboxFlag},
	)
...
func (c *CmdSetRestartPolicy) setLocalFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(
		&c.restartPolicyOptions.SandboxName,
		sandboxFlag,
		util.MainClusterSandbox,
		"The name of the sandbox, in which the restart policy will be set "+
			"(the main cluster will be set by default).",
	)
}
...
func (c *CmdSetRestartPolicy) Run(vcc vclusterops.ClusterCommands) error {
	...
	sandboxMsg := ""
	if c.restartPolicyOptions.SandboxName != util.MainClusterSandbox {
		sandboxMsg = fmt.Sprintf("in sandbox %q", c.restartPolicyOptions.SandboxName)
	}

	vcc.DisplayInfo("Successfully set restart policy as %q %s",
		c.restartPolicyOptions.Policy, sandboxMsg)
