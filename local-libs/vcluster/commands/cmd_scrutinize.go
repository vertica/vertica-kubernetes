...
func (c *CmdScrutinize) setLocalFlags(cmd *cobra.Command) {
	...
	cmd.Flags().IntVar(
		&c.sOptions.ScrutinizeFileSizeLimit,
		"scrutinize-file-size-limit",
		vclusterops.ScrutinizeFileLimitGB,
		"The maximum size, in GB, of non-log files to collect."+
			util.Default+fmt.Sprint(vclusterops.ScrutinizeFileLimitGB),
	)
...
func (c *CmdScrutinize) validateParse(logger vlog.Printer) error {
	...
	if (c.sOptions.LogFileSizeLimit < 0) || (c.sOptions.ScrutinizeFileSizeLimit < 0) {
		return fmt.Errorf("both log-file-size-limit and scrutinize-file-size-limit should be greater than 0 or 0 for unlimited size")
	}
