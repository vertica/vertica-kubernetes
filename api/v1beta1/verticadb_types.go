...
type VerticaDBSpec struct {
	...
	// +kubebuilder:validation:Optional
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// Descriptors of the init containers as well as corresponding volumes that
	// will be used to prepare runtime environments for programs in the main
	// container, e.g. UDx language runtimes.  Multiple init containers may be
	// specified and will be started in the same order as their descriptors.
	// Multiple (emptyDir) volumes may be specified for each init container,
	// and each volume may be mounted at different locations in the init
	// container as well as the main container.
	Init []InitDescriptor `json:"init,omitempty"`
...
// InitDescriptor is used to provide specifications for an init container
// which prepares runtime environments for programs inside the main container.
type InitDescriptor struct {
	// +kubebuilder:validation:Required
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// The name of the init container, must be unique within the Pod.
	Name string `json:"name,omitempty"`
	// +kubebuilder:validation:Required
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// The Docker image used for the init container.
	Image string `json:"image,omitempty"`
	// +kubebuilder:validation:Required
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// The commands that will be executed by "/bin/sh" when the init
	// container starts.
	Command string `json:"command,omitempty"`
	// +kubebuilder:validation:Required
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// A list of pairs of paths that specify mount points of volumes attached
	// to the init container.
	Paths []InitPaths `json:"paths,omitempty"`
}

// InitPaths defines a pair of paths which are mount points of a volume
// in an init container and the main container respectively.
type InitPaths struct {
	// +kubebuilder:validation:Required
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// The path inside the init container where a volume is mounted
	InitPath string `json:"initPath,omitempty"`
	// +kubebuilder:validation:Required
	// +operator-sdk:csv:customresourcedefinitions:type=spec,xDescriptors="urn:alm:descriptor:com.tectonic.ui:advanced"
	// The path inside the main container where a volume is mounted
