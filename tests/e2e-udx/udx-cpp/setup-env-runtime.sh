...
do
    ...
    then
        ...
        then
           ...
           sudo yum install -y boost-devel libcurl-devel bzip2-devel bzip2 perl diffutils && ln -s /usr/bin/python3 /usr/bin/python && ln -s /usr/lib64/libbz2.so /usr/lib64/libbz2.so.1.0 && break || sleep 60
        fi
    else  # v2 image
        if which apt-get # ubuntu / apt-get
        then
