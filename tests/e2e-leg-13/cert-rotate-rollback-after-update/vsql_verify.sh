...
if [[ $CERT_NAME != "https_cert_3" ]]; then
    echo "ERROR: cert https_cert_3 not found"
    exit 1
fi
...
if [[ $CA_CERT_NAME != "https_ca_cert_3" ]]; then
    echo "ERROR: cert https_ca_cert_3 not found"
    exit 1
fi
KEY_NAME=$(kubectl exec -n $NAMESPACE v-cert-rotate-rollback-after-update-sc1-0 -c server -- vsql -tAc "select name FROM cryptographic_keys WHERE secret_name='custom-cert';")
if [[ $KEY_NAME != "https_key_3" ]]; then
    echo "ERROR: key https_key_3 not found"
    exit 1
fi
exit 0
